...
export default function ScreeningFlow({ language }: ScreeningFlowProps) {
  ...
  async function finalize(probsList: number[][]) {
    try {
      ...
      if (demographics) {
        saveSession(
          ...
          rfAnswers,
          rfProbs,
          nlpResults.length ? nlpResults : null,
          res.diagnosis,
          symptomSelections
        ).catch(() => {});
      }
    ...
    return (
      <div className="flex items-center justify-center py-24">
        <div className="text-center">
          ...
          <p style={{ color: "var(--text-muted)" }}>Initialising CogniDetect AI...</p>
        </div>
      </div>
    );
  }
